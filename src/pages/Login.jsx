...
import { adminLogin } from "../apis/auth"; // ADMIN login
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // res is { success: true, data: { admin, token, message } }
      if (!res || !res.data?.token || !res.data?.admin) {
        throw new Error("Invalid response from server");
      }

      // Build the admin object we save in AuthContext (admin fields + token)
      const adminObject = {
        ...res.data.admin,
        token: res.data.token,
      };

      // Save in Auth Context (also saves to localStorage)
      setLoginData({ admin: res.data.admin, token: res.data.token });

      // Redirect after login (admin dashboard route)
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Enter admin ID (e.g. superadmin)"
              disabled={isLoading}
            />
          </div>
