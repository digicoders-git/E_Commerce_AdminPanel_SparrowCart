...
export const getAllProductsAPI = () => {
  return http.get(`/products/admin/list/all`);
};

// Get products by category
export const getProductsByCategoryAPI = (categoryId) => {
  return http.get(`/products/admin/list/all?categoryId=${categoryId}`);
};
...
export const updateProductAPI = (productId, formData) => {
  return http.patch(`/products/${productId}`, formData, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
  });
...
export const updateProductStatusAPI = (productId, isActive) => {
  return http.patch(`/products/${productId}/status`, { isActive });
};
